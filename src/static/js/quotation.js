...
function makeQuotationForBox() {
  ...
  if (result) {
    ...
    price.innerHTML = result.fee.toFixed(1) +  " DH";
    price.innerHTML = `
    <span class="price-text">Total:</span>
    <span class="price-total">DH ${result.fee.toFixed(1)} </span>
    <span class="price-unit">(DH ${(result.fee / Number(number.value)).toFixed(
      2
    )} / unit)</span>
    `;
...
function makeQuotationForMockup() {
  ...
  const price = document.querySelector("#price-box");
  price.innerHTML = "DH " + fee.toFixed(1);
  price.innerHTML = `
  <span class="price-text">Total:</span>
  <span class="price-total">DH ${fee.toFixed(1)} </span>
  `;
}
